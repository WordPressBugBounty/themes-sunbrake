(function ($) {
  ...
  function getSiteLeadsBackendData(path, defaultValue) {
    return _.get(top.sunbrakethemeSiteLeadsData, path, defaultValue);
  }
  ...
  const setIsLoadingText = text => {
    const infoNoticeNode = document.querySelector('.sunbraketheme-siteleads-integration-button__notice--info');
    if (!infoNoticeNode) {
      return;
    }
  ...
  const setIsLoading = newValue => {
    const buttonNode = document.querySelector('.sunbraketheme-siteleads-integration-button');
    if (!buttonNode) {
      return;
    }
    if (newValue) {
      buttonNode.classList.add('sunbraketheme-loading');
    } else {
      buttonNode.classList.remove('sunbraketheme-loading');
      hideInfoNotice();
    }
  };
  ...
  function onUpdateButtonTextAndListeners() {
    buttonNode = document.querySelector('.sunbraketheme-siteleads-integration-button');
    if (!buttonNode) {
      return;
    }
  ...
  function onUpdateNoticeText() {
    let noticeNodeContainer = document.querySelector('.sunbraketheme-siteleads-integration-button__container');
    if (!noticeNodeContainer) {
      return;
    }
    let titleNode = noticeNodeContainer.querySelector('.sunbraketheme-siteleads-integration-button__title');
    ...
    let descriptionNode1 = noticeNodeContainer.querySelector('.sunbraketheme-siteleads-integration-button__description_1');
    descriptionNode1.innerHTML = getTranslatedText('notice_description_1');
    let descriptionNode2 = noticeNodeContainer.querySelector('.sunbraketheme-siteleads-integration-button__description_2');
    descriptionNode2.innerHTML = getTranslatedText('notice_description_2');
    let descriptionNode3 = noticeNodeContainer.querySelector('.sunbraketheme-siteleads-integration-button__description_3');
    descriptionNode3.innerHTML = getTranslatedText('notice_description_3');
  }
  function unUpdateNoticeErrorText() {
    hideInfoNotice();
    let noticeErrorNode = document.querySelector('.sunbraketheme-siteleads-integration-button__notice--error');
    if (!noticeErrorNode) {
      return;
    }
  ...
  //Siteleads init
  function initApp() {
    let container = document.querySelector('.sunbraketheme-siteleads-integration-button__container');
    if (!container) {
      return;
    }
